...
/**
 * Wrapper for LWA AuthorizationManager.
 */
class AuthorizationManagerWrapper {

    private static final String TAG = AuthorizationManagerWrapper.class.getName();

    /**
     * Performs login with LWA.
     *
     * @param request AuthorizeRequest login request.
     */
    void authorize(final AuthorizeRequest request) {
     ...
     * @param listener     sign out Listener.
     */
    void signOut(final MainActivity mainActivity, final Listener<Void, AuthError> listener) {
        if (mainActivity != null && listener != null) {
            AuthorizationManager.signOut(mainActivity, listener);
        } else {
            Log.e(TAG, ERROR_LOGOUT);
        }
    }
     ...
     */
    private boolean validateRequest(final AuthorizeRequest request) {
        return request != null;
    }
}
