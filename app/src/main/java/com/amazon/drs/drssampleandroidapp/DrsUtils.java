...
import static com.amazon.drs.drssampleandroidapp.Constants.MAX_LENGTH;
import static com.amazon.drs.drssampleandroidapp.Constants.PATTERN;
import static com.amazon.drs.drssampleandroidapp.Constants.UTF8_ENCODING;

/**
 * @hide Utility class for performing String and buffered stream operations.
 */
final class DrsUtils {

    private static final String TAG = DrsUtils.class.getName();

    /**
     * Private constructor. DrsUtils should not be instantiated by other classes.
     */
    private DrsUtils() {
    }

    /**
     * Builds the buffered reader for release prior to Kitkat.
     *
     * @param inputStream Stream received from the UrlConnection.
     * @return BufferReader of the input stream.
     */
    private static BufferedReader getBufferedReaderOld(final InputStream inputStream) {
        try {
            return new BufferedReader(new InputStreamReader(inputStream, UTF8_ENCODING));
    ...
    /**
     * Builds the buffered reader.
     *
     * @param inputStream Stream received from the UrlConnection.
     * @return BufferReader of the input stream.
     */
    private static BufferedReader getBufferedReader(final InputStream inputStream) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT) {
            ...
            return getBufferedReaderOld(inputStream);
        }
    }
     ...
     */
    static String getStringFromStream(final InputStream inputStream) throws IOException {
    ...
    /**
     * Validates if the search string satisfies the character limit.
     *
     * @param searchString the string that needs to be checked.
     * @return true if the input matches the regular expression else false.
     */
    static boolean isValidInput(final String searchString) {
        try {
            final Pattern pattern = Pattern.compile(PATTERN);
            final Matcher matcher = pattern.matcher(searchString);
            return (matcher.matches() && searchString.length() < MAX_LENGTH);
